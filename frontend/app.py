...
def main():
    ...
    with st.sidebar:
        ...
        page = st.selectbox(
            "Choose a page:",
            ["<� Home", "=� Upload Documents", "S Ask Questions", "=� Analytics Dashboard", "� Settings"]
        )
        ...
        try:
            ...
            if health_response.status_code == 200:
                ...
                if active_trackers > 0:
                    st.markdown(f'<span class="real-time-update">=� {active_trackers} active jobs</span>', unsafe_allow_html=True)
            else:
...
def show_upload_page():
    st.subheader("=� Document Upload with Real-Time Progress")
    ...
    if documents:
        ...
        display_df.columns = ['=� Document', '=� Status', '=" Chunks', '=� Uploaded']
    ...
    for i in range(300):  # 5 minutes max
        ...
        if progress_data:
            ...
            if status == "completed":
                ...
                details_placeholder.info(f"� Completed at: {timestamp}")
                break
            elif status == "failed":
                error = progress_data.get("error", "Unknown error")
                status_placeholder.error(f"L Processing failed: {error}")
                details_placeholder.error(f"� Failed at: {timestamp}")
                break
...
def show_document_details(doc_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            with col2:
                st.metric("=� File Size", f"{doc_data.get('file_size', 0):,} bytes")
                st.metric("=� File Type", doc_data.get('file_type', 'Unknown').upper())
                ...
                if response.status_code == 200:
                    ...
                    if result["sources"]:
                        ...
                        for i, source in enumerate(result["sources"]):
                            with st.expander(f"=� Source {i+1} - {source.get('source_file', 'Unknown Document')}"):
                                col1, col2 = st.columns([3, 1])
...
def show_analytics_dashboard():
    ...
    try:
        ...
        if documents:
            ...
            st.info("=� No analytics data available yet. Upload some documents to see metrics!")
            
            # Show sample dashboard
            st.markdown("### =� Sample Analytics View")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            info_display = {
                ...
                "=� Real-time Features": "Server-Sent Events + Progress Tracking"
            }
